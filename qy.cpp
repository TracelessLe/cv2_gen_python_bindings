...
static struct PyModuleDef qy_moduledef =
{
    PyModuleDef_HEAD_INIT,
    MODULESTR,
    "Python wrapper for QuYan.",
    -1,     /* size of per-interpreter state of the module,
               or -1 if the module keeps state in global variables. */
    special_methods
};

PyMODINIT_FUNC PyInit_qy();
PyObject* PyInit_qy()
{
    import_array(); // from numpy
    PyObject* m = PyModule_Create(&qy_moduledef);
    if (!init_body(m))
        return NULL;
    return m;
}
